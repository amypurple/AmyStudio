export function inferControllerBackendFromSource(sourceText) {
  const codeText = String(sourceText || "")
    ...
    .join("\n");
  if (/\bspinner\s*\(/i.test(codeText)) return null;
  const ports = {
  ...
  if (controllerBackend === "bios_cont_scan_compact" && /\b(?:play\s+sound|play\s+song|set\s+sound\s+table|on\s+(?:vblank|frame)|spinner|120c)\b/i.test(codeText)) return null;
  return {
    controllerBackend,
...
export function inferAmyMemoryCapabilities(sourceText, sourceHintsTinySound) {
  ...
  const needsSleepState = /\bsleep\s+after\s+(?:[0-9]+|\$[0-9A-F]+|[A-Za-z_][A-Za-z0-9_]*)\s+seconds?(?:\s+on\s+joypad\s+[12])?/i.test(codeText);
  ...
  const usesJoypadReleased2 =
    ...
    usesDynamicJoypadReleased;
  const needsSpinner =
    /\b(spinner|AMY_(ENABLE_SPINNER|DISABLE_SPINNER|RESET_SPINNER1|RESET_SPINNER2|RESET_SPINNERS)|SPINNER_[12])\b/i.test(text);
