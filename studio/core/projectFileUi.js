...
export function createProjectFileUiHelpers({
  ...
  function pictureGroupNameFromPath(path) {
    ...
    const withoutCodec = file.replace(/\.(zx0|zx1|zx2|zx7|aplib|dan1|dan2|dan3|pletter|plet5|lzf|rle|mdkrle|bitbuster|nibble)$/i, "");
  ...
  function insertProjectFilePictureSnippet(entry) {
    ...
    const targetPrefix = normalizeProjectFilePath(entry.path)
      .replace(/\.(zx0|zx1|zx2|zx7|aplib|dan1|dan2|dan3|pletter|plet5|lzf|rle|mdkrle|bitbuster|nibble)$/i, "")
    ...
    const group = (project.projectFiles || []).filter((candidate) => {
      ...
      const withoutCodec = normalized.replace(/\.(zx0|zx1|zx2|zx7|aplib|dan1|dan2|dan3|pletter|plet5|lzf|rle|mdkrle|bitbuster|nibble)$/i, "");
  ...
  async function evaluatePictureCompressionCandidatesWithWorkers(tables, codecs) {
    const requestedCodecs = Array.isArray(codecs) && codecs.length
      ? codecs
      : ["raw", "mdkrle", "nibble", "zx0", "zx1", "zx2", "aplib", "zx7", "dan2", "dan1", "dan3", "pletter", "bitbuster", "lzf"];
