...
- Replaced z88dk's raw bitmap tables with a verified project-level MDKRLE path, reducing it from 14,293 to 5,911 bytes.
- Revalidated all seven Warrior ROMs with exact VRAM and pixel output and refreshed the evidence archive.
- Kept ugBASIC's native image result unchanged because no safe Coleco direct-to-VRAM compression path was verified.
...
- Allowed several tuple combinations to share one `case` body.
...
- Added inferred-length `put Buffer at X,Y` and centered writes for qualified arrays.
...
- Added same-type whole-record `=` and `<>` comparisons for static, nested, overlay, and dynamically indexed operands using inline byte scans with no helper RAM.
...
- Added same-type whole-record assignment for static records, nested record fields, overlay parts, and dynamically indexed record-array elements using inline `LDIR` with no helper RAM.
...
- Extended qualified byte operands to canonical `get char` destinations, `play sound`, `stop sound`, and canonical typed printing.
...
- Extended single-field sprite setters/getters and named-hitbox collisions to accept byte-sized runtime index expressions while preserving direct-address codegen for constants.
- Added `joypad(N).fire` for either standard fire button and `joypad(N).action` for any of four action buttons; both use one controller read and one mask.
- Added typed `data Name records Type` ROM templates and exact `copy Template to RecordArray` validation/codegen, emitting one `LDIR` and rejecting type or row-count mismatches.
- Added checked fixed-size slices from larger typed record tables, allowing one data-driven level loader while rejecting partial destinations and record-type mismatches.
- Added logical pixel-space `box ... size ... collides with box ... size ...` conditions with short-circuited AABB codegen and no VRAM reads; Fly Swatter now uses it for its tile-rendered boss.
- Consolidated all five Fly Swatter wave starters into one table-driven routine, then replaced five centered-message routines with one fixed-width ROM text table; its Balanced ROM size fell from 5880 to 5610 bytes (and from 6125 before the earlier record-template refactor).
- Extended `put Source count N at X,Y` to accept general addressable sources such as `Data + Offset`, enabling compact runtime-selected text and tilemap rows.
- Reviewed Fly Swatter as a public teaching example: table-driven boss transitions, one shared timer shutdown, compact completed-wave logic, removed duplicate drawing, and comments for timing, centering, input edges, and tile-to-pixel collision; Balanced ROM is now 5381 bytes.
- Added the in-Studio Optimization Cookbook comparing explicit assignments, DATA cursor streams, and typed `LDIR` templates with practical tradeoffs and measured Fly Swatter results.
- Corrected CRT-safe pauses so the current screen remains unchanged until timeout, then blanks to black and restores the tracked VDP backdrop and display state after a fresh action.
- Rewrote Fly Swatter actor updates and collision checks with the general forms, reducing its Balanced ROM from 6209 to 6125 bytes before documentation-only changes.
- Added compiler regressions, autocomplete entries, syntax highlighting, and the complete language-reference contract and limitations.
## 2026-08-07 - ROM-table state dispatch

- Replaced the linear compare ladders generated by `on Expr goto/gosub` with bounded 1-based ROM address tables and direct `jp (hl)` dispatch.
- Added an indirect-call trampoline for `on Expr gosub`, preserving normal returns without exposing writable function pointers or adding runtime helpers.
- Kept `0` and out-of-range selectors as fallthrough, rejected unknown targets as before, and widened selectors before indexing so tables above 128 entries remain correct.
- Documented the atomic `u8` selector pattern for main/NMI state machines and added ASM regressions for byte, word, gosub, and 130-target dispatch.
...
## 2026-08-04 - Amy source syntax coloring

- Replaced textual `Preview` actions in FILES with the same compact purple-eye symbol used by CV/DINA previews, retaining file-specific hover titles and accessible labels.

- Removed preview filters from ROM TEST & DEBUG so the emulator displays its native 256x192 framebuffer directly, avoids loading the filter module through this path, and eliminates per-frame filter post-processing.

- Added presentation-only syntax coloring for Amy keywords, types, built-ins, directives, numbers, strings, comments, operators, and inline ASM.
- Added context-aware TMS9918A categories: VDP/display vocabulary uses TMS blue and frame units use light green.
- Recognize project only as a quoted project directive, and pattern/color/name only in VDP contexts, keeping custom variables neutral.
- Color parentheses, brackets, and expression punctuation consistently as operators.
- Audited compiler context against the highlighter: metadata declarations now share magenta, legal identifier names remain neutral, and missing statement/qualifier/time vocabulary is classified by grammatical role.
- Added a persistent `{ }` source-toolbar toggle; disabling syntax colors stops tokenization and restores native textarea rendering without reloading.
- Replaced the temporary `{ }` control with a compact Atari-era inspired `COLOR` hardware switch while preserving modern keyboard and screen-reader semantics.
- Default new browsers to legacy monochrome source and compact the optional switch label to `CLR`; previously saved preferences remain respected.
- Removed the historically ambiguous `CLR` label, localized the color/colour tooltip, and changed the lever from metal to glossy ColecoVision-black plastic.
- Shortened the moving lever and removed the state-colored outer capsule so the control stays vertically centered without changing the SOURCE card layout.
- Unified the splash, ColecoVision wordmark, and Amy syntax categories around the six-color ColecoVision BIOS rainbow instead of unrelated custom token colors.
- Restored purple Amy branding, kept the historical rainbow exclusive to ColecoVision, aligned SOURCE action spacing with the debugger tabs, and compacted breakpoint actions to accessible icon buttons.
- Reduced the splash to `AMY STUDIO`, `LOADING IDE...`, and one progress bar; its background and sweep now use explicit cyan, orange, and yellow TMS9918 colors while purple remains reserved for Amy.
- Refined syntax colors so `nmi on` and `nmi off` share the VDP color, variable types use TMS light blue, and strings use the lighter TMS orange.
- Shortened decorative generated-ASM separators and section headings while retaining build, memory, vector, and instruction-level diagnostics.
- Removed the redundant source-editor help sentence because the same shortcuts already appear in the SOURCE toolbar.
- Replaced the visible `COMPILE` toolbar label with a compact, freely licensed Lucide hammer icon while preserving the `Compile ROM` hover tooltip and accessible button label.
- Replaced the Compile gear with the Lucide hammer icon so build and settings actions use distinct, conventional symbols.
- Harmonized the build tooltip as `Compile ROM` and rebuilt the splash progress bar from six equal ColecoVision-logo color bands with a neutral animated highlight.
- Enlarged the splash `AMY STUDIO` title from 54px to a responsive 70px maximum, centered it, and widened the card without allowing mobile overflow.
- Strengthened the splash loading sweep with a wider white core and cyan halo while slightly reducing the centered title and card.
- Removed duplicated splash CSS from `index.html`, deleted obsolete loading-step/hint rules, and consolidated reduced-motion handling; the compact Compile SVG remains inline.
- Kept the native source textarea as the sole editing surface, preserving selection, caret, autocomplete, breakpoints, IME behavior, and compiler input.
- Added a shared editor adapter for programmatic text, selection, replacement, focus, scroll, and change notifications.
- Synchronized the color layer with horizontal and vertical source scrolling and virtualized its DOM to the visible lines plus a small margin.
- Added tokenizer, HTML-escaping, adapter, and large-source performance regressions; all 177 Studio examples still transpile.
...
- Added `nibble` to the picture and charset/tile compression choosers; `aplib` remains excluded because no Amy Studio ColecoVision VRAM decompressor is shipped.
- Re-ran the Warrior pattern/color codec roundtrip benchmark across the 10 available compressors. The quick import pass now evaluates `raw` plus the 5 fastest compressors from that test: `mdkrle`, `nibble`, `bitbuster`, `zx7`, and `dan1`; full compare still tests all 10 compressors. The chooser now labels browser decompression time as JS verification and sorts runtime speed through a separate Z80/VDP class instead of pretending JavaScript timing predicts ColecoVision timing.

## 2026-07-03 - Legacy-aligned NMI and VDP wait contracts

**Summary:** Re-aligned Amy Studio's ColecoVision runtime with the proven legacy devkit NMI/VDP behavior.

- Changed generated NMI code so VDP status is always acknowledged before any `$701F` critical-section skip, matching the legacy `crtcv.s` contract.
- Kept decompression and VDP critical sections global/runtime-safe instead of requiring per-demo `READ_REGISTER` workarounds.
- Updated NMI enable/disable, screen mode setup, sprite mode setup, and direct display on/off codegen to keep the `$73C4` VDP register shadow coherent for `wait`.
- Simplified `wait`/delay NMI-off polling to follow the legacy delay structure more closely.
- Removed the unnecessary `AMY_FILL_VRAM` alias and route fills directly to the Coleco BIOS `FILL_VRAM` routine.

## 2026-07-02 - Smooth Criminal music/equalizer demo

**Summary:** Added an Amy port of the legacy `sdcctest/smoothc` music/equalizer demo.

- Converted Smooth Criminal name, pattern, sprite-pattern, and color graphics to ZX0 project assets.
- Ported the original Coleco BIOS sound table from `music.s` into the Amy listing.
- Recreated the original VBlank music timeline and sprite equalizer effect.
- Verified `smooth-criminal-music` at `3952 bytes` and full `node tools/check-examples.mjs` (130/130).
## 2026-07-02 - Portal graphics unified on ZX0

**Summary:** Recompressed all Portal demo graphics project files as ZX0.

- Converted Portal name, pattern, sprite-pattern, and color data to `.zx0` project assets.
- Updated Portal to use `decompress zx0` for every graphics upload, including `vram.spr_pat`.
- Extended the Amy decompression target parser to accept sprite VRAM tables.
- Verified Portal at `2680 bytes` and full `node tools/check-examples.mjs` (129/129).
## 2026-07-02 - Portal demo promoted to complete

**Summary:** Marked the Portal demo port as complete after runtime confirmation.

- Removed the incomplete label from the Portal catalog entry and source comments.
- Added the working Portal demo to the clean AmyStudio sample set.

## 2026-07-01 - Portal demo initial Amy port

**Summary:** Added an incomplete Amy port of the legacy `sdcctest/portal` effect demo.

- Converted the original MDKRLE name/pattern/sprite-pattern tables into Studio project files and expanded the legacy 32-byte `load_color` table into a full compressed VRAM color table.
- Added a `Portal Demo (Incomplete)` catalog entry using the original BIOS sound table from `music.s`.
- Preserved the original VBlank equalizer idea by reading Coleco BIOS sound areas and updating sprite attributes each frame.
- Verified `node tools/check-examples.mjs --only portal-demo` and full `node tools/check-examples.mjs` (129/129).

## 2026-07-01 - Santa Gift Run marked incomplete

**Summary:** Marked Santa's Gift Run as an incomplete Amy port and recorded its original source path.

- Updated the example header and Studio catalog entry to identify `old-projects-sdcc-z80/sdcctest/giftrun/main.c` as the source.
- Noted that converted graphics and BIOS sounds are present, while gift-drop and house behavior still need closer matching.

## 2026-07-01 - Legacy devkit porting checklist

**Summary:** Documented the historical porting contracts that Santa's Gift Run exposed.

- Added `docs/legacy-devkit-porting-checklist.md` covering Mode 2 pattern thirds, two-name-table buffering, WAV2CV/Marcel sound conversion, and NMI sound playback contracts.
- Expanded the Amy language reference for `set screen pages` / `swap screens`, including the old `screen(display, write)` mapping and the warning about direct `vram.name` writes.
- Expanded the WAV2CV-to-Coleco-BIOS note with recognition rules for `start_sound(..., area)` and `update_sound()` plus Santa-style volume-swept note conversion.

## 2026-07-01 - Santa Gift Run screen page flipping

**Summary:** Restored visible coverage for the legacy two-screen-buffer trick and applied it to Santa's Gift Run gameplay rendering.

- Re-added the old devkit page-flip demo to the Studio catalog as `Old Devkit Page Flip`.
- Updated Santa's Gift Run to render each frame on the hidden name table and reveal it with `swap screens`.
- Replaced direct `vram.name` clearing with current-page row fills so drawing follows the active screen page.

## 2026-07-01 - Santa Gift Run pattern thirds fix

**Summary:** Duplicated the Santa's Gift Run gameplay charset across all mode-2 text pattern thirds.

- Added `duplicate mode 2 text patterns` after loading `GiftRunGamePattern` so lower screen rows use the intended tiles.
- Verified `node tools/check-examples.mjs --only santas-gift-run` and full `node tools/check-examples.mjs` (127/127).

## 2026-07-01 - Santa Gift Run drop sound fix

**Summary:** Corrected the Santa's Gift Run drop-gift sound effect command.

- Replaced the rough drop-gift bytes with the intended `$81,$8F,$90,$1E,$16,$01` sweep and `$90` terminator.
- Verified `node tools/check-examples.mjs --only santas-gift-run` and full `node tools/check-examples.mjs` (127/127).

## 2026-07-01 - Santa Gift Run sound conversion fix

**Summary:** Corrected the Santa's Gift Run note-data conversion to proper Coleco BIOS sound commands.

- Rebuilt the title theme as volume-swept tone commands (`$82`) that fold each note and following rest into one duration.
- Applied the same note/rest sweep conversion to the chimney success phrase.
- Verified `node tools/check-examples.mjs --only santas-gift-run` and full `node tools/check-examples.mjs` (127/127).

## 2026-07-01 - Santa's Gift Run initial Amy port

**Summary:** Added a first Amy Studio port of the legacy Santa's Gift Run project.

- Added a converter for the legacy ColecoPaint `TITLE` and `GAME` picture streams and generated ZX0 project assets.
- Added a playable simplified Amy listing with converted title/game graphics, tile-frame sleigh/house/gift rendering, score/houses HUD, and BIOS sound table entries.
- Registered the sample in the Studio catalog as `Santa's Gift Run`.
- Verified `node tools/check-examples.mjs --only santas-gift-run` and full `node tools/check-examples.mjs` (127/127).

## 2026-07-01 - 421 marked incomplete

**Summary:** Kept 421 in the full development catalog as an incomplete archival port and confirmed it is excluded from the clean AmyStudio release.

- Marked the full catalog entry as `421 Dice (Incomplete)` so it is not mistaken for a finished game.
- Documented that the preserved parts are the HUD, ICVGM graphics, dice display, and converted BIOS sounds; the full game rules remain future work.
- Confirmed the clean AmyStudio repository contains no 421 sample entry.

## 2026-07-01 - 421 HUD-respecting rollback

**Summary:** Removed the experimental interactive 421 round logic and restored the HUD-driven dice display.

- Restored the compact 421 flow that loads the designed NAME table and redraws only the six dice with `put DiceFrame frame size 3,3`.
- Kept original-author credits and the CRT-safe player-one dice offset matching the corrected NAME table.
- Removed the added turn/selection/scoring logic that did not respect the existing HUD.
- Verified `node tools/check-examples.mjs --only 421` and full `node tools/check-examples.mjs` (126/126).

## 2026-07-01 - 421 CRT-safe NAME table

**Summary:** Moved the 421 player-one board art out of the CRT overscan edge in the actual generated NAME table.

- Updated `tools/convert-421-assets.mjs` to shift the player-one labels, die numbers, dice, and TOTAL label one tile right during asset generation.
- Regenerated `421.name.zx0` in the Studio project files and set `FirstDieX = 2` so runtime die redraws match the corrected NAME table.
- Removed the runtime overlay that wrote at X=0; the data itself is now safe.
- Verified `node tools/check-examples.mjs --only 421` and full `node tools/check-examples.mjs` (126/126).

## 2026-07-01 - 421 original credits and overscan labels

**Summary:** Credited Mathieu Proulx and Daniel Bienvenu in the 421 port and made board labels CRT-overscan safe.

- Added menu/instructions credits for Mathieu Proulx's original idea/project and Daniel Bienvenu's devkit/tools/coding work.
- Rewrites the loaded board labels at X=1 after decompression so useful PLAYER/TOTAL text is not hidden by CRT overscan.
- Verified `node tools/check-examples.mjs --only 421` and full `node tools/check-examples.mjs` (126/126).

## 2026-07-01 - 421 playable dice round

**Summary:** Turned the 421 devkit port from a random dice display into an interactive two-player dice round.

- Replaced keypad-number menu input with a joypad up/down/fire selector.
- Added per-player dice selection: blue dice are rerolled, red dice are kept.
- Added up-to-three-roll turn flow, player one/player two evaluation, and a simplified 421 ranking model covering 421, two ones, triples, runs, and normal values.
- Verified `node tools/check-examples.mjs --only 421` and full `node tools/check-examples.mjs` (126/126).

## 2026-07-01 - 421 title picture assets

- Files browser picture preview now accepts 32-byte text-mode color tables by expanding them to the per-tile-row format used by the preview renderer.

**Summary:** Restored the original 421 bitmap title screen as modern Amy Studio project files.

- Converted the legacy `titre.c` full-screen RLE picture into `421-title.pattern.zx0` and `421-title.color.zx0` using `tools/convert-421-assets.mjs`.
- Attached the converted title pattern/color files plus the 421 board name/pattern/color ZX0 tables to the catalog entry so the Studio Files panel can expose the project assets.
- Changed the 421 startup flow to `show picture FourTwentyOneTitle` before the text menu.
- The 421 text menu now explicitly calls `screen on` after rebuilding the menu, so display/NMI/input/sound resume after leaving the title picture.
- Replaced visible source includes with `asset ... from "@project/..." codec zx0`, `decompress zx0 ...`, and an inline Snake-style sound table block.
- Forced character `$20` in the 421 board pattern table to a blank 8-byte pattern and added inline comments to `FourTwentyOneDiceFaces` identifying the blue/red die faces 1-6.
- Verified `node tools/check-examples.mjs --only 421`, direct exported ASM assembly (5488 bytes), and full `node tools/check-examples.mjs` (126/126).


## 2026-07-01 - 421 text-mode color loading

**Summary:** Corrected the 421 port to use true text-mode color loading instead of the legacy expanded `load_color` path.

- Switched the 421 board setup from `tile screen` to `text screen`.
- Changed `FourTwentyOne_LoadColor` to copy exactly the 32 text-mode color bytes to `VRAM_COLOR`.
- Removed the Mode 2 expanded color-table digit fill that only applied to the simulated text mode path.
- Re-verified the 421 export and direct assembly; ROM size is now 4751 bytes.

## 2026-07-01 - 421 legacy devkit Amy port

**Summary:** Added a playable Amy Studio port of Daniel Bienvenu's 2002 legacy devkit 421 dice demo.

- Added `examples/ports/devkit/421.alexis` with title/menu/rules pages and dice rolls rendered through `put ... frame size 3,3`.
- Converted the ICVGM name/pattern/color tables from `old-projects-sdcc-z80/sdccold/421/icvgm.c` into `examples/ports/devkit/data/421_graphics.inc`.
- Added small 421-specific ASM helpers for MDKRLE graphics load, legacy `load_color` expansion, and dice-face staging into a RAM frame buffer.
- Registered the example in the Studio games catalog.
- Verified `node tools/check-examples.mjs --only 421`, full `node tools/check-examples.mjs` (126/126), sound-table validation, and direct assembly of the exported 421 ASM (4996 bytes).

## 2026-07-01 - WAV2CV generated sound index annotations

**Summary:** Made generated WAV2CV-to-BIOS sound includes easier and safer to use from Amy code.

- Each generated sound-table entry now comments its final `play sound N` index.
- Generated files include an `Amy play sound indexes` summary block.
- The converter emits a warning comment if a forced layout makes entry 1 target anything other than sound area 1.

## 2026-07-01 - Smarter numbered WAV2CV music placement

**Summary:** Improved the WAV2CV-to-BIOS converter so repeated numbered multi-channel groups are treated as music by default.

- A group such as `song0`, `song1`, `song2` marked with `--repeat song` now maps to sound areas 1-3 automatically.
- Added `--sfx-prefix` and `--no-auto-music` so repeated numbered groups can still be forced into SFX areas when needed.
- Preserved source-order SFX numbering when no numbered music group is detected.

## 2026-07-01 - Legacy WAV2CV BIOS sound converter

**Summary:** Added a standalone converter for old WAV2CV/Marcel-style C sound arrays into Coleco BIOS/lib4ksa sound table includes for Amy Studio ports.

- Added `tools/convert-wav2cv-to-coleco-bios.mjs`.
- Generated `examples/generated/421-sounds-bios.asm` from `old-projects-sdcc-z80/sdccold/421/sounds_.c`.
- Documented channel mapping, dummy lower sound-area reservation, numbered multi-channel sounds, and 421 porting notes in `docs/legacy-wav2cv-to-coleco-bios-porting.md`.
- Verified the generated 421 table with `tools/validate-sound-table.mjs`.
# AMY Changelog
...
## 2026-07-01 — Integer Random Runtime Seed Fix

**Summary:** Replaced integer `random(N)` / `random(A, B)` codegen's inline `GET_RANDOM` sequence with a compact `AMY_RANDOM_U8` wrapper around the Coleco BIOS random routine, using the shared `$73C8` seed and a zero-seed guard.

**Changes**
- Added `AMY_RANDOM_U8` as a granular runtime module, linked only when integer random expressions are used.
- `random(0, 240)` and similar byte ranges now use `GET_RANDOM` plus the legacy Z80 refresh-register mix, but with a guaranteed non-zero seed and correct preservation of the resulting `A` value through array stores.
- Updated the random expression selftest expected values for the current runtime sequence.

**Verification**
- Verified Cosmo Trainer export: `AMY_RANDOM_U8` is called, `GET_RANDOM` is defined, and `A` is preserved before storing into `Stones[I].X`.
- `node tools/check-examples.mjs --only cosmo-trainer-port,amy-random-expression-test,amy-float-ahl-benchmark,amy-float-random-lab,amy-float-random-range-lab`: 5 passed, 0 failed.
- `node tools/check-examples.mjs`: 125 passed, 0 failed.

---
## 2026-07-01 — FP5 Random Range Assignment

**Summary:** Added `fp5` support for `Var = random(min, max)` while preserving the existing fp5 random generator sequence.

**Changes**
- `fp5` assignments and declaration initializers now accept `random(A, B)` and emit `A + random() * (B-A)` using one `AMY_FP5_RND` call.
- Added `amy-float-random-range-lab` to cover constant bounds, fp5 variable bounds, negative ranges, and declaration initialization.
- Updated autocomplete and the AMY language reference.

**Verification**
- `node tools/check-examples.mjs --only amy-float-random-range-lab,amy-float-random-lab,amy-float-ahl-benchmark`: 3 passed, 0 failed.
- `node tools/check-examples.mjs`: 125 passed, 0 failed.

---

## 2026-07-01 — Archive Inactive Studio Measure Snapshots

**Summary:** Moved monolithic `_tmp_measure_app*.mjs` snapshots out of the active `studio/` source tree.

**Changes**
- Archived the two tracked measurement snapshots under `docs/archive/2026-07-01-studio-tmp-measure-snapshots/`.
- Added an archive README clarifying that active compiler modules live under `studio/core/compiler/` and `build-preview/core/compiler/`.

**Verification**
- Confirmed no active references remain outside the archive.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.

---
## 2026-07-01 — BCD Print Coordinate Codegen Cleanup

**Summary:** Folded constant BCD print digit coordinates at codegen time instead of emitting repeated `inc e` column steps.

**Changes**
- `print BcdValue at x,y` now emits direct per-digit `DE` coordinates when `x` and `y` are compile-time constants.
- Variable-coordinate BCD printing keeps the previous `ld d/y`, `ld e/x`, `inc e` fallback.

**Verification**
- Targeted examples: 8 passed, 0 failed.
- Targeted post-built-in oracle on BCD-heavy examples: 11 succeeded, 0 failed; total built-in ROM size dropped by 214 bytes across the targeted set.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.

---
## 2026-07-01 — CRT-Safe Example Print Positions

**Summary:** Shifted sample text that started at screen column 0 to column 1.

**Changes**
- Updated code samples, featured examples, build-preview examples, and autocomplete snippets so non-centered `print ... at 0,y` text starts at `x = 1`.
- Left `print centered at 0, ...` unchanged because that argument is the row, not the X coordinate.

**Verification**
- `rg --pcre2` confirms no non-centered `print ... at 0,` remains in `studio`, `build-preview`, or `examples`.
- `node --check` passed on all changed `.js` and `.mjs` files.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.

---

## 2026-07-01 — Page-Safe HL Increment Peephole

**Summary:** Recovered sequential `HL` stores after page-local address narrowing.

**Changes**
- Added a Balanced local-value-reuse peephole that rewrites `ld l,next` to `inc hl` only when `L` is locally proven to be `next - 1` and the increment cannot cross a page boundary.
- Uses `inc hl` rather than `inc l` because `inc hl` preserves flags like `ld l,n`.

**Verification**
- `node tools/check-examples.mjs --only amy-selftest,amy-feature-test,brinquitos-game-demo,cvbasic-demo-port`: 4 passed, 0 failed.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.
- Targeted oracle: `amy-selftest` and `amy-feature-test` each shrink by 3 bytes versus the previous post-built-in oracle state.

---

## 2026-07-01 — Balanced Oracle-Driven Optimizer Cleanup

**Summary:** Reduced the full post-built-in oracle gap with conservative flag-aware peepholes and source-level codegen cleanup.

**Changes**
- Added carry-liveness-gated `sub 1 -> dec a` and `add a,1 -> inc a` folds. These fire only when Carry is overwritten or unreachable before any carry consumer.
- Improved generated NMI controller shadow writes by storing contiguous `JOYPAD_1`, `KEYPAD_1`, `JOYPAD_2`, `KEYPAD_2` through one `HL` walk instead of reloading `HL` for each shadow.
- Reordered sound/random startup init so the dummy sound table pointer remains live for the initial `SET_SOUND_TABLE` call, removing a redundant `ld hl,AMY_SOUND_TABLE_DUMMY`.
- Avoided unnecessary `push af` / `pop af` around constant stack/record/array address calculations when the address setup does not touch `A`.
- Added a strict direct-store recovery peephole for `ld a,imm; ld hl/l/h,...; ld (hl),a` when `A` is proven dead after the store.

**Verification**
- `node tools/check-examples.mjs`: 124 passed, 0 failed.
- Full post-built-in oracle: 124 succeeded, 0 failed; total built-in ROM size across the corpus is 497 bytes smaller than `post-builtin-oracle-true-full-2026-07-01`.
- MDL residual dropped from 330 bytes to 241 bytes; copt remains unchanged / 0.
- Largest wins include `quick-sort-bars` -51 bytes, `brinquitos-game-demo` -45 bytes, `bubble-sort-bars` -33 bytes, `cvbasic-viboritas-port` -24 bytes, and `diamond-dash` -21 bytes.

---

## 2026-07-01 — Numeric Codegen Cleanup

**Summary:** Cleaned up the most obvious numeric codegen waste identified by the MDL residual case audit.

**Changes**
- Reordered numeric `str$` buffer formatting so destination address setup happens before loading the source/value into `HL`; this removes dead `ld hl,Var` emissions and matches the formatter ABI more clearly.
- Made 32-bit immediate stores avoid repeated `ld a,sameByte` loads inside one emitted literal store sequence.
- Replaced one local numeric postprocess `cp 1` with `dec a` where only the Z flag is consumed by the following branch.

**Verification**
- `node tools/check-examples.mjs --only amy-numeric-regression-demo,amy-numeric-edge-cases-demo,amy-numeric-overflow-demo,amy-numeric-str-lab,amy-u32-multiply-lab`: 5 passed, 0 failed.
- Targeted post-built-in oracle on those five examples: 5 succeeded, 0 failed; MDL residual is 21 bytes. `amy-numeric-str-lab` residual dropped to 1 byte.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.

---

## 2026-07-01 — FP5 Sqrt Dead Register Cleanup

**Summary:** Removed one dead `ld b,a` from the fp5 square-root runtime source.

**Changes**
- Removed an obsolete exponent copy in `coleco_math_fp5_sqrt.asm`; only the exponent parity in `C` is used, and `B` is overwritten by the subsequent 31/32-step shift loop before any read.
- Regenerated `studio/core/alexisLibrarySources.generated.js` so Studio uses the updated fp5 sqrt runtime.

**Verification**
- `node tools/check-examples.mjs --only amy-float-sqrt-lab,amy-float-sqrt-precision-lab,amy-float-sqrt-byte-lab,amy-float-builtin-surface-lab,amy-float-ahl-benchmark`: 5 passed, 0 failed.
- Targeted post-built-in oracle on those five examples: `Remove unused ld b,a` disappeared from the hotspot list.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.
- `node tools/check-examples.mjs --test-types`: passed except intentional out-of-scope `bcd *=` and `bcd /=`.
- Full post-built-in oracle: MDL residual dropped from 74 to 69 bytes; copt remains unchanged / 0.

---
## 2026-07-01 — Balanced 8-Bit Register Immediate Reuse

**Summary:** Generalized constant register loads so Balanced can reuse another 8-bit register that is locally proven to already hold the requested immediate value.

**Changes**
- Added a bounded local proof for known 8-bit register constants, including direct `ld r,n`, 16-bit pair loads, and `xor a` for `A=0`.
- Added a Balanced+ rewrite: `ld dst,n` -> `ld dst,src` when `src` is proven equal to `n`. Both forms preserve flags; the rule stops at labels, branches, calls, returns, `djnz`, `ex`, `exx`, and writes to the source register.
- Kept the older complete-removal rules for truly redundant loads; this new rule handles the MDL family like `ld d,0 -> ld d,l` when the target was not already known.

**Verification**
- `node --check studio/vendor/amyscvassembly/optimizerCore.js`: passed.
- Targeted fixed/fixed32/numeric examples: 11 passed, 0 failed.
- Targeted oracle: `ld d,0 -> ld d,l` disappeared from the hotspot list.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.
- `node tools/check-examples.mjs --test-types`: passed except intentional out-of-scope `bcd *=` and `bcd /=`.
- Full post-built-in oracle: MDL residual dropped from 89 to 74 bytes; copt remains unchanged / 0.

---

## 2026-07-01 — Balanced Constant Store Peephole Gate

**Summary:** Kept the conservative `ld a,n; ld (hl),a -> ld (hl),n` peephole, but moved it behind the Balanced local-value-reuse gate.

**Changes**
- The rule still fires only when `A` is proven overwritten before any later read, label, branch, call, or local analysis barrier.
- Safe no longer applies this value-reuse fold; Balanced/Aggressive/Experimental do.
- This preserves a simpler Safe mode for debugging while keeping the byte-saving rule available in normal optimized builds.

**Verification**
- `node --check studio/vendor/amyscvassembly/optimizerCore.js`: passed.
- `node tools/check-examples.mjs --only amy-float-muldiv-lab,amy-float-sqrt-lab,amy-float-ahl-benchmark`: 3 passed, 0 failed.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.
- Safe-level targeted oracle on the same three examples: 3 succeeded, 0 failed.

---

## 2026-07-01 — FP5 Multiply Constant Store Cleanup

**Summary:** Removed a redundant accumulator load in the fp5 multiply runtime source.

**Changes**
- Replaced `ld a,1; ld (hl),a` with `ld (hl),1` in the fp5 multiply overflow-normalization path.
- Kept the change in the library source instead of adding a broad peephole, because `A` is dead immediately after this store.
- Regenerated `studio/core/alexisLibrarySources.generated.js` so Studio uses the corrected fp5 runtime.

**Verification**
- `node tools/check-examples.mjs --only amy-float-muldiv-lab,amy-float-log-lab,amy-float-exp-lab,amy-float-sqrt-lab,amy-float-sqrt-precision-lab,amy-float-sqrt-byte-lab,amy-float-builtin-surface-lab,amy-float-ahl-benchmark`: 8 passed, 0 failed.
- Targeted post-built-in oracle on the same eight examples: `ld a,?const1; ld (hl),a` disappeared from the hotspot list.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.
- Full post-built-in oracle: MDL residual dropped from 97 to 89 bytes; copt remains unchanged / 0.

---

## 2026-06-30 — Random Runtime Xorshift Cleanup

**Summary:** Removed a redundant high-byte store/reload directly from the fp5 and fixed32 random runtime helpers.

**Changes**
- Simplified the final xorshift step in `coleco_math_fp5_random.asm` and `coleco_math_fx16_random.asm`.
- Preserved the same seed handling and random sequence; the value already in `A` is reused instead of storing it to `H` and loading it back.
- Regenerated `studio/core/alexisLibrarySources.generated.js` so Studio uses the corrected library sources.

**Verification**
- `node tools/check-examples.mjs --only amy-fixed32-random-lab,amy-float-random-lab,amy-float-ahl-benchmark,amy-float-print-lab,amy-float-builtin-surface-lab,amy-float-str-buffer-lab`: 6 passed, 0 failed.
- Targeted post-built-in oracle on the same six examples: `Remove unused ld h,a` disappeared from the MDL hotspot list.
- Full post-built-in oracle: MDL residual dropped from 103 to 97 bytes; copt remains unchanged / 0.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.
- `node tools/check-examples.mjs --test-types`: passed except intentional out-of-scope `bcd *=` and `bcd /=`.

---

## 2026-06-30 — Record Array Put-Char Codegen

**Summary:** Specialized `put char RecordArray[I].Tile at RecordArray[I].X,RecordArray[I].Y` so the transpiler computes the record-array element address once instead of relying on aggressive dead-code cleanup later.

**Changes**
- Added a strict `put char` fast path for three `u8` fields from the same `record_array` element and same index expression.
- The generated code now loads `D=Y`, `E=X`, and `A=Tile` by walking field offsets from one `HL` base address.
- The generic `put char` path remains unchanged for all other operands.

**Verification**
- `node tools/check-examples.mjs --only record-array-minimal,nested-record-array-minimal`: 2 passed, 0 failed.
- Targeted post-built-in oracle: both record labs now have 0 MDL residual bytes; ROM sizes dropped from 316/318 to 274/276 bytes.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.
- Full post-built-in oracle detailed scope: MDL residual dropped from 135 to 103 bytes; copt remains unchanged / 0.

---

## 2026-06-30 — Strict HL High-Byte Reuse Peephole

**Summary:** Added a conservative peephole for partial HL constant reloads.

**Changes**
- Added `ld hl,nn -> ld l,low(nn)` only when a local scan proves `H` already equals `high(nn)`.
- The proof stops at labels, branches, calls, or writes/clobbers of `HL`; it deliberately does not reproduce MDL cases that require proving array-index ranges after `add hl,de`.

**Verification**
- `node tools/check-examples.mjs --only cvbasic-demo-port,bubble-sort-bars,insertion-sort-bars,heap-sort-bars,quick-sort-bars,three-sort-algorithms`: 6 passed, 0 failed.
- Targeted oracle audit on the same 6 examples: MDL residual is 29 bytes; the new rule captures the locally safe case while range-dependent `ld hl,const` cases remain for future codegen/range analysis.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.
- `node tools/check-examples.mjs --test-types`: passed, except intentional out-of-scope `bcd *=` and `bcd /=`.

---

## 2026-06-30 — Wider Safe LD D,0 Deduplication

**Summary:** Improved the safe peephole proof for redundant `ld d,0` in generated array-addressing code.

**Changes**
- Extended the local `D=0` lookback window from 8 to 32 instructions while still stopping at labels, barriers, calls, or any write/clobber of `D`.
- Confirmed the remaining MDL `Remove duplicate ld d,0` hotspot came from generated Amy array/indexing code in CVBasic and sorting demos, not from included ASM runtime routines.

**Verification**
- `node tools/check-examples.mjs --only cvbasic-demo-port,bubble-sort-bars,insertion-sort-bars,heap-sort-bars,quick-sort-bars,three-sort-algorithms`: 6 passed, 0 failed.
- `tools/audit-post-builtin-oracles.ps1` on the same 6 examples: MDL residual dropped to 30 bytes and `Remove duplicate ld d,0` disappeared from the hotspot table.
- `node tools/check-examples.mjs --test-types`: passed, except intentional out-of-scope `bcd *=` and `bcd /=`.

---

## 2026-06-30 — Routine ABI Table for Compiler Codegen

**Summary:** Added a compiler-side routine ABI table so runtime and BIOS calls can share one register/clobber contract.

**Changes**
- Added `studio/core/compiler/routineAbi.js` as the conservative source of truth for runtime/BIOS inputs, outputs, and clobbers.
- Routed runtime call clobber metadata through the ABI table.
- Added `routineRegisterLoadHelpers.js` so new codegen can load ABI-declared inputs without repeating ad hoc HL/BC/DE staging logic.
- Added `tools/check-routine-abi.mjs` to fail when compiler-emitted routine calls lack ABI metadata.

**Verification**
- `node tools/check-routine-abi.mjs`: passed; 120 compiler-emitted call/jp targets covered.
- `node tools/check-examples.mjs --test-types`: passed, except intentional out-of-scope `bcd *=` and `bcd /=`.
- `node tools/check-examples.mjs`: 124 passed, 0 failed.

---

## 2026-06-23 — Safe Named Amy Timers

**Summary:** Added static, state-friendly Amy timers without exposing the unsafe parts of BIOS timer allocation.

**Changes**
- Added `timer Name every N ticks` and `timer Name after N ticks [stopped]` declarations.
- Added `start timer Name`, `stop timer Name`, and `if timer Name then Statement`; `if timer` tests and consumes the signal in normal code rather than calling game logic from NMI.
- Added NMI-side static timer updates only when a program declares timers. No dynamic timer allocation, no `FREE_SIGNAL`, and no hidden `every N frames` scheduler.
- Added `on vblank SubName` as the canonical spelling for the VBlank hook; `on frame SubName` remains accepted as an alias.
- Added `Amy Timer Lab` to exercise a repeating timer and a stopped one-shot timer.
- Fixed first-pass NMI inference so `on vblank` / `on frame` makes `screen on` enable interrupts even in programs that do not also use `wait` or `frame`.
- Added a smaller hook-only NMI path for programs that use only `on vblank`: no alternate-register `exx` wrapper, hook runs in the normal register set, and NMI exits keep using compact `ret`.

**Verification**
- `node --check studio/core/compiler/transpileAmyCore.js`: passed.
- `node --check studio/core/compiler/transpileFinalizationHelpers.js`: passed.
- `node --check studio/core/project.js`: passed.
- `node --check studio/examples.js`: passed.
- `node tools/check-examples.mjs --only amy-timer-lab`: `1 passed, 0 failed out of 1 examples`.
- `node tools/export-studio-examples-asm.js --only amy-timer-lab --out-dir build/scratch-timer-lab --manifest-out build/scratch-timer-lab/manifest.json`: passed; exported ASM contains per-timer `active/count/reload/signal` labels and NMI `AMY_TIMER_DONE_*` update blocks.
- `node tools/check-examples.mjs --only amy-timer-lab,amy-on-frame-lab,cvbasic-vgm-player-port`: `3 passed, 0 failed out of 3 examples`.
- Fixed `Amy Timer Lab` timing so the stopped one-shot timer fires after the second repeating timer event instead of stopping `BlinkTimer` too early.
- Made `Amy Timer Lab` human-readable: `BlinkTimer` now fires every 30 ticks and `DoorTimer` fires after 120 ticks with visible status text.
- Made `Amy On Frame Lab` human-readable and renamed its source text to the canonical `on vblank` wording: it now counts visibly to 120 ticks before PASS.
- `node tools/check-examples.mjs`: `123 passed, 0 failed out of 123 examples`.
- `node tools/export-studio-examples-asm.js`: `123 ok, 0 failed`; exported `amy-on-frame-lab.asm` contains `AMY_NMI_HOOK_READ_STATUS`, `call AMY_UPROC_TickFrame`, `AMY_SCREEN_ON_NMI`, no `exx` in the NMI body, and compact NMI exits with `ret`.
- Minimal inference check: `on vblank Tick` without `wait` reports `needsNmi: true` and `usesVblankHook: true`.

---

## 2026-06-22 — CVBasic VGM Player Port

**Summary:** Added an Amy version of CVBasic `vgm.bas` as a self-contained example.

**Changes**
- Added `CVBasic VGM Player Port`, using Amy `on frame VgmPlay` to mirror CVBasic `ON FRAME GOSUB vgm_play`.
- Embedded a tiny SN76489 VGM stream directly in the example because `alexkidd.vgm` is not present beside the CVBasic source in this workspace.
- Kept the low-level PSG write path visible through inline ASM (`out ($ff),a`) so the demo documents the current gap around `VARPTR`/ROM `PEEK`/`OUT`/`INCBIN`-style binary VGM assets.

**Verification**
- `node --check studio/examples.js`: passed.
- `node tools/check-examples.mjs --only cvbasic-vgm-player-port`: `1 passed, 0 failed out of 1 examples`.
- `node tools/export-studio-examples-asm.js --only cvbasic-vgm-player-port --out-dir build/scratch-cvbasic-vgm-port --manifest-out build/scratch-cvbasic-vgm-port/manifest.json`: passed; exported ASM contains `call AMY_UPROC_VgmPlay`, `VgmMusic`, and `out ($ff),a`.

---

## 2026-06-22 — NMI Runtime Contract and On-Frame Hook

**Summary:** Documented the generated NMI contract and added a safe Amy-level per-frame hook.

**Changes**
- Added `docs/nmi-runtime-contract-2026-06-22.md`, documenting the current Amy NMI variants, the `$701F` VDP I/O guard, the legacy `crtcv.s` comparison, and the `on frame` hook contract.
- Implemented `on frame SubName`: a top-level declaration that registers a parameterless Amy sub called from the generated VBlank NMI wrapper with full register preservation and a `NO_NMI` reentrancy guard.
- Added `Amy On Frame Lab` plus harness metadata validation for the generated hook target.

**Verification**
- `node tools/check-examples.mjs --only amy-on-frame-lab`: `1 passed, 0 failed out of 1 examples`.
- `node tools/export-studio-examples-asm.js --only amy-on-frame-lab --out-dir build/scratch-on-frame-lab --manifest-out build/scratch-on-frame-lab/manifest.json`: passed; full ASM contains `Nmi:`, `call AMY_UPROC_TickFrame`, `AMY_SKIP_USER_FRAME_HOOK`, `NO_NMI`, `push ix`, and `push iy`.
- `node tools/check-examples.mjs --only amy-conditional-compile-lab,amy-conditional-cstyle-lab,amy-on-frame-lab`: `3 passed, 0 failed out of 3 examples`.
- `node tools/check-examples.mjs`: `121 passed, 0 failed`, including the new on-frame lab.

---

## 2026-06-21 — Legacy-Shaped Bounded Random

**Summary:** Adjusted bounded random code generation so retry-heavy games like
Diamond Dash keep the legacy-style entropy timing of the Coleco random helper.

**Changes**
- Verified Diamond Dash `restore HolesShape1/HolesShape2` plus
  `read TmpDX, TmpDY` generates the expected DATA cursor reads and signed
  byte table values.
- Changed `random(A, B)` and bounded byte random helpers to preserve loop
  registers around `GET_RANDOM`, matching the older `rnd_byte` timing shape
  more closely instead of using an overly-tight retry loop.
- Updated Studio cachebusters so the browser and clean release load the fixed
  compiler modules.
- Removed a stale `end sub` from Diamond Dash that emitted a second `Start:` label before the sound include and could corrupt the cartridge entry/symbol map.
- Verified the fixed Diamond Dash compile has one `Start:` and coherent MDKRLE symbols (`mdkrle_decompress=$8105`, `mdkrle_set_write_addr=$8128`).
- Hardened the transpiler so an obsolete/orphan `end sub` cannot close the implicit `Start` block and create a second `Start:` label.
- Moved plain `include "..."` handling before implicit executable `Start` opening so data/sound includes do not become accidental code blocks.
- Added a conservative unreachable-ASM cleanup pass after unconditional `ret`/`jp`/`jr`, stopping at labels and data/include directives.
- Added source-level unreachable pruning inside explicit routines: code after a root-level `return`/`goto` is discarded until a reachable routine/global label, preventing debug-shortcut code from still entering the ROM.
- Added compile-time conditionals (`define`, `ifdef`, `ifndef`, `else ifdef`, `end ifdef`, plus C-style `#define/#ifdef/#else/#endif`) so temporary debug blocks and alternate assets can be excluded before variables, DATA, assets, includes, and helpers are scanned.
- Fixed routine/include boundary handling so a stale `end sub` before `include "@project/diamond_sounds.inc"` in Diamond Dash produces byte-identical exported ASM to the modern no-`end sub` form.
- Added `Amy Conditional Compile Lab`, a regression example for `define`/`ifdef`/`ifndef` where active and inactive branches define same-name subs; inactive branches must not create duplicate symbols or emitted code.
- Extended `tools/check-examples.mjs` with ASM-level validation hooks for conditional labs, so the harness fails if inactive branch code/text leaks despite successful compilation.
- Added `Amy C-Style Conditional Lab` for `#define/#ifdef/#else/#endif`, including an inactive nested `#define` that must not leak into later branches.
- Fixed `tools/check-examples.mjs --only id1,id2` so targeted validation actually filters examples and reports unknown ids.

**Verification**
- `node tools/export-studio-examples-asm.js --only diamond-dash --out-dir build/scratch-diamond-current --manifest-out build/scratch-diamond-current/manifest.json`
- `node tools/check-examples.mjs` -> `118 passed, 0 failed`
- `node tools/export-studio-examples-asm.js --out-dir build/scratch-start-label-scan` -> `118 ok`, every exported ASM has exactly one `Start:` label.
- Diamond Dash end-sub equivalence test: exported ASM is identical with or without `end sub` before `include "@project/diamond_sounds.inc"`; both variants keep one `Start:` and one sound include.
- `node tools/export-studio-examples-asm.js --only amy-conditional-compile-lab --out-dir build/scratch-conditional-compile-lab2 --manifest-out build/scratch-conditional-compile-lab2-manifest.json`: passed; inactive branch strings and `Result = 99` are absent from ASM, active `Result = 42` is present.
- `node tools/check-examples.mjs --only amy-conditional-compile-lab,amy-conditional-cstyle-lab`: `2 passed, 0 failed out of 2 examples`.
- `node tools/check-examples.mjs --only does-not-exist`: fails with `Unknown or non-Amy example id(s): does-not-exist`.
- `node tools/check-examples.mjs`: `120 passed, 0 failed`, including both conditional lab ASM validation hooks.
...
## 2026-06-21 — Restore Legacy-Grade Random Range Masking

**Summary:** Fixed bounded integer random codegen so Amy uses the same
power-of-two mask/retry shape as the legacy `rnd_byte` helper instead of
retrying against the full 0..255 byte for every range.

**Changes**
- `random(A, B)` with compile-time byte bounds now emits a mask for the next
  power-of-two range (`and $1F` for 26 values, `and $0F` for 14 values, etc.)
  before the rejection compare.
- `random(N)` with compile-time byte counts now uses the same masked retry
  strategy.
- This restores the practical behavior needed by Diamond Dash and Brinquitos:
  random placement no longer depends on very rare `< range` hits from an
  unmasked 8-bit sample.

**Verification**
- `node tools/export-studio-examples-asm.js --only diamond-dash,brinquitos-game-demo --out-dir build/scratch-random-mask-fix --manifest-out build/scratch-random-mask-fix/manifest.json`: passed.
- `node tools/check-examples.mjs --only diamond-dash,brinquitos-game-demo`: passed.
- `node tools/check-examples.mjs`: 118/118 examples passed.
...
- Added `tile screen` as the full Mode 2 tile/text bootstrap:
  ...
  `duplicate mode 2 text patterns`, first-third 2KB color initialization, and
  `cls`.
