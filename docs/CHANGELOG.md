...
- Expanded the GearColeco overlay self-test to 21 runtime assertions, covering a 7-byte record stride, constant and variable indices, `u16`, signed `i8`, the field immediately following a record array, qualified mutations, qualified ascending/descending loop counters, and qualified `for each` in all five optimization profiles.
