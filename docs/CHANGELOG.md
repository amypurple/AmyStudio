...
## 2026-08-07 - Public documentation and timer game example

- Added visible links to the public [Amy Studio repository](https://github.com/amypurple/AmyStudio) and documentation from both the README and Studio menu.
- Added `Fly Swatter: Timer Quest`, a playable example using named timers and explicit game phases across three fly rounds and a multi-phase boss fight.
- Expanded the timer safety self-test screen so its expected 17 passes and zero failures are explained before the measured result is shown.
- Kept the language reference factual: named timers use `tick`/`ticks`, while PAL/NTSC-aware seconds remain specific to supported input-blanking commands.
