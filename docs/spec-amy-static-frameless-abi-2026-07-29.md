...
4. **A parameterized Amy routine directly entered from inline ASM keeps the IX stack ABI**, even when the transfer target is known. The compiler does not rewrite the ASM caller's argument marshalling, so converting only the callee would mix calling conventions. A parameterless ASM target may remain frameless when the ordinary recursion and NMI proofs succeed.
...
11. **ASM calling-convention fence.** A direct inline-ASM call/jump to a parameterized Amy routine must keep that callee on the IX stack ABI; assert both analyzer eligibility and emitted callee prologue. A parameterless direct ASM target remains eligible when no other fence applies.
...
- **RAM budget:** the transpiler and `amyc` now report static-ABI parameter bytes, local bytes, total bytes, and routine count. The current Reversi v5 export measures 62 bytes (14 parameter bytes + 48 local bytes) across 28 frameless routines; the offsetting stack reduction keeps the net RAM cost modest.
