...
To compose without an existing file, click `SOUND`, then `New Tiny Song`. Studio prepares two valid eight-note channels; name and insert the song, then edit and audition its notes in the sequencer. `Add after` duplicates the selected note and `Delete` removes it. Channel tempo controls the step length; per-note rhythm editing remains future work.
