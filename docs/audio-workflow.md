...
To compose without an existing file, click `SOUND`, then `New Tiny Song`. Studio prepares two valid eight-note channels; name and insert the song, then edit and audition the notes in the sequencer. This starter workflow is intentionally small: adding/removing steps and editing rhythm remain future sequencer work.
