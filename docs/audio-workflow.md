...
Coleco BIOS sound composition remains technical because its compact commands directly describe PSG periods, attenuation, duration, sweeps, and sound-area behavior. Amy Studio provides a Sound FX editor inside the source sound inspector: select a command, change its note/noise, exact PSG period, channel, volume, duration, frequency sweep or volume sweep, audition it, replace it, then play and save the complete sequence. Duration `256` is shown normally even though the BIOS encodes it as `$00`. Commands can be reordered by dragging or with the arrow buttons. Full song arrangement remains future work.
