...
For Tiny Sound music stored as `_ch1` and `_ch2`, choose `Sequencer` beside channel 1. Both channels appear as synchronized columns with duration-scaled notes, rests, holds, a shared `Play`/`Pause`/`Stop` transport, and a playhead that highlights the active events. The channel instrument shows its exact `$02` bytes and editable volume, decay step/count, initial delay, and repeat rate. Select a plain note to edit and preview its pitch. Amy Studio changes only the selected source bytes; arpeggios and special commands remain read-only until their exact encoding is fully covered.
