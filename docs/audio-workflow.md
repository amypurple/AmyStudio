...
For Tiny Sound music stored as `_ch1` and `_ch2`, choose `Sequencer` beside channel 1. Both channels appear as synchronized columns with duration-scaled notes, rests, holds, and a playhead that highlights active events. Select a note, then use `Here` to play from that frame or `Loop` to repeat its range; a voice already sounding on the other channel resumes at its correct pitch and envelope state. Click a channel heading to edit its `$02` volume and decay envelope inline, or click a plain note to edit its pitch, vibrato and arpeggio. `Cancel` or `Escape` discards the pending edit. Amy Studio changes only the selected source bytes; special commands remain read-only until their exact encoding is fully covered.
