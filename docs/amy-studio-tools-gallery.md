...
Tiny Sound entries show their tempo, instrument, musical notes, sustain, silence, special commands, and loop instead of the misleading single `Tiny` command. `Listen Tiny channel` auditions one channel; matching `_ch1` and `_ch2` labels also offer paired playback.
The paired Tiny transport provides `Play 2 channels`, `Stop`, and `Play again`, synchronized on one preview clock. Tiny data remains read-only until Amy Studio can re-encode every command without changing its meaning.
