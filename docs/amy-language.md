...
declare a separate top-level `on vblank` hook. Frame handlers run in NMI:
...
- `ref u32` / `ref i32` parameters mutate addressable wide variables without copying
...
know the VDP timing contract you are using. At top level, use `on vblank`; `on frame`
is reserved for the handler binding inside a `scene` declaration.
...
with scene frame handlers, top-level `on vblank`, controllers, music, and sound. Projects that never use either
