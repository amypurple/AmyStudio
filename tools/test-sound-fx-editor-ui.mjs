...
assert.match(source, /applyComposer\.textContent = "Apply changes"/, "Selected command edits need a nearby apply action");
...
assert.match(source, /undoStack/, "Sound FX sequence edits must be undoable");
assert.match(source, /redoStack/, "Undone Sound FX edits must be restorable");
