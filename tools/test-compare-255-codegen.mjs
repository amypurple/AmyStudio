...
u8 Failures = 0
Value = 254
if Value = 255 then Failures += 1
if Value <> 255 then Failures += 0 else Failures += 1
Value = 255
if Value <> 255 then Failures += 1
if Value = 255 then Failures += 0 else Failures += 1
Value = 0
...
const output = await mkdtemp(join(tmpdir(), "amy-compare-255-"));
try {
  ...
  for (const profile of profiles) {
    ...
    assert.match(asm, /ld a,\(AMY_UVAR_Value\)\s+inc a\s+jp [zn]z,/i, `${profile}: missing compare-to-255 specialization`);
    assert.doesNotMatch(asm, /ld a,\(AMY_UVAR_Value\)\s+cp (?:255|\$FF)/i, `${profile}: retained CP 255`);
    const failureMatch = asm.match(/^AMY_UVAR_Failures\s+EQU\s+\$([0-9A-Fa-f]+)/m);
    assert.ok(failureMatch, `${profile}: missing Failures symbol`);
    rom[0] = 0x55;
    rom[1] = 0xaa;
    const core = await GearcolecoTestCore.create({ seed: 0x323535 });
    try {
      core.loadBios(bios);
  ...
  console.log("Compare-to-255 specialization PASS (5 profiles, runtime checked)");
} finally {
