...
try {
  ...
  try {
    ...
    assert.equal(core.readRam(passedAddress, 1)[0], 7, "Overlay runtime assertions did not all pass.");
  } finally {
    core.destroy();
  }
  console.log(`Amy overlay ROM self-test PASS (${optimization}: aliases and record-array stride retained, 7 assertions)`);
} finally {
