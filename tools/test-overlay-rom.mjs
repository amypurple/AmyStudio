...
try {
  ...
  try {
    ...
    assert.equal(core.readRam(passedAddress, 1)[0], 39, "Overlay runtime assertions did not all pass.");
  } finally {
    core.destroy();
  }
  console.log(`Amy overlay ROM self-test PASS (${optimization}: aliases, arrays, double indexes, VRAM I/O, collisions, mutations, loops, and BCD retained, 39 assertions)`);
} finally {
